...
async function fetchData3() {
    const response = await fetch('/3datas');
    if (!response.ok) {
...
async function fetchData23() {
    const response = await fetch('/23datas');
    if (!response.ok) {
...
async function updateConfirm(messageId) {
    ...
    const response = await fetch(`/confirm/${messageId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            confirmStatus: true  // 새로운 confirmStatus 값
        })
    });
    ...
    if (response.ok) {
        ...
        console.error('confirmStatus 업데이트 실패', response.statusText);
    }
}
...
async function fetchData() {
    ...
    firstmessages.forEach(fm => {
        ...
        if (fm.confirmStatus === 0 && relatedAnswers.length > 0) {
...
async function sendMessage(messageId) {
    ...
    const response = await fetch(`/sending/${messageId}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
